...
) RETURNING created_at, client_id, category, token_hash, redirect_uri, user_id, scopes, data, expires, used, code_challenge
`
...
SELECT created_at, client_id, category, token_hash, redirect_uri, user_id, scopes, data, expires, used, code_challenge FROM oauth WHERE category = $1 AND token_hash = $2 AND expires > $3
`
